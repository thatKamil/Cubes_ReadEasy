...
def openLogFileAndProcess():
    '''Begins the main function of the program. Select file and pass to various functions'''
...
if len(sys.argv) == 1:  # TRUE result indicates file hasn't been dropped onto the program icon. Wait for user input.
    # ASCII art displayed in text window.
    textArea.insert(END, '\n\n\n\t\t    +------+         +------+  \n\t\t   /|     /|        /|     /|\n\t\t'
                         '  +-+----+ |       +-+----+ |\n\t\t  | |    | |       | |    | |\n\t\t  | +----+-+       '
                         '| +----+-+\n\t\t  |/     |/        |/     |/\n\t\t  +------+         +------+  \n')
else:
    # File has been dropped onto the program icon. Directs to file location. Determines the type of file format and
    # passes it to a further function.
